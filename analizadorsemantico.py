...
def evalAsignacion(arbol,estado):
  ...
  if obj.tipo == 'matriz':
    try:
      if (obj.fila != obj.valor.shape[0]) or (obj.columna != obj.valor.shape[1]):
        raise Exception ('El tamano de la matriz no corresponde a la asignada')
      else:
        if not(isinstance(obj.valor, np.ndarray)):
          raise Exception('Error de tipos')
    except:
      raise Exception('Error de tipos')
  elif obj.tipo == 'real':
    if isinstance(obj.valor, np.ndarray):
      raise Exception('Error de tipos')
